...
Public Class frmTransactionHistory
    ...
    Private Sub frmTransactionHistory_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Button3.Visible = (currentuser.Role = ROLE_SUPERVISOR)   ' Q41: only the Supervisor may cancel
        LoadGrid("")
    End Sub
                ...
                Using localDr As MySqlDataReader = localCmd.ExecuteReader()
                    ...
                    While localDr.Read()
                        DataGridView1.Rows.Add(
                            ...
                            localDr("product_code").ToString(), localDr("product_name").ToString(), localDr("size").ToString(),
                            Convert.ToDecimal(localDr("unit_price")).ToString("N2"), localDr("qty").ToString(),
                            Convert.ToDecimal(localDr("subtotal")).ToString("N2"), Convert.ToDecimal(localDr("total_amount")).ToString("N2"),
                            Convert.ToDecimal(localDr("amount_paid")).ToString("N2"), Convert.ToDecimal(localDr("amount_change")).ToString("N2"),
                            localDr("status").ToString(), localDr("username").ToString())
                    End While
                End Using
            End Using
            cn.Close()

            Label1.Text = ChrW(8369) & Convert.ToDecimal(If(ExecScalar(
                "SELECT IFNULL(SUM(total_amount),0) FROM TBL_TRANSACTIONS WHERE transaction_no LIKE @s AND status <> 'Cancelled'",
                New String() {"@s"}, New Object() {"%" & searchText & "%"}), 0)).ToString("N2")
    ...
    Private Sub Button2_Click(sender As Object, e As EventArgs) Handles Button2.Click   ' View Details
        If DataGridView1.SelectedRows.Count = 0 Then
            MsgBox("Select a transaction row first.", vbExclamation, "Transaction") : Exit Sub
        End If
        Dim txnNo As String = DataGridView1.SelectedRows(0).Cells("TransactionNo").Value.ToString()
    ...
    Private Sub Button3_Click(sender As Object, e As EventArgs) Handles Button3.Click   ' Cancel Transaction
        ...
        End If
        Dim txnNo As String = DataGridView1.SelectedRows(0).Cells("TransactionNo").Value.ToString()
        If DataGridView1.SelectedRows(0).Cells("Status").Value.ToString() = "Cancelled" Then
            MsgBox("This transaction is already cancelled.", vbInformation, "Transaction") : Exit Sub
        End If
        ...
        Try
            ...
            For Each r As DataRow In items.Rows
                ExecNonQuery("UPDATE TBL_PRODUCT_VARIANTS SET quantity_on_hand = quantity_on_hand + @q WHERE variant_id = @v",
                    New String() {"@q", "@v"}, New Object() {Convert.ToInt32(r("qty")), Convert.ToInt32(r("variant_id"))})
            Next

            ExecNonQuery("UPDATE TBL_TRANSACTIONS SET status = 'Cancelled', cancel_reason = @r WHERE transaction_id = @id",
                New String() {"@r", "@id"}, New Object() {reason, transactionId})

            MsgBox("Transaction cancelled and stock restored.", vbInformation, "Transaction")
            LoadGrid(txtSearch.Text.Trim())
        Catch ex As Exception
